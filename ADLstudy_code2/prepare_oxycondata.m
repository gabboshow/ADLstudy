switch machine
    ...
    %Linux
    case 0
        path_data_oxicon = strcat('/media/Elements/Gabriele/ADL_study/All/Patient_',num2str(Partindex),'/SensorData/');
    %Windows
    case 1
        path_data_oxicon = strcat('E:\Gabriele\ADL_study\All\Patient_',num2str(Partindex),'\SensorData\');
...
oxycon_onsets = zeros(1,numel(1:(1/fs_oxycon):respiration_onsets(end)));
oxycon_onsets((fs_oxycon-1)*respiration_onsets) = 1;
index_onsets = find(oxycon_onsets==1);

square_resp = zeros(1,numel(1:(1/fs_oxycon):respiration_onsets(end)));
...
square_resp = [zeros(fs_oxycon*30*60,1)', square_resp, zeros(fs_oxycon*30*60,1)'];

time_plot = (0:numel(square_resp)-1)/fs_oxycon;
figure
